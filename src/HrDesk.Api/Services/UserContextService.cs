...
public class UserContextService
{
    public UserContext? ExtractFromClaims(ClaimsPrincipal? user, string correlationId)
    {
        ...
        var employeeId = user.FindFirst(HrDesk.Core.Constants.ClaimTypes.EmployeeId)?.Value ?? user.FindFirst("sub")?.Value ?? "Unknown";
        var email = user.FindFirst(System.Security.Claims.ClaimTypes.Email)?.Value ?? "unknown@company.com";
        var country = user.FindFirst(HrDesk.Core.Constants.ClaimTypes.Country)?.Value ?? "US";
